...
To build packages on-the-fly and directly from source on github,
quelpa can be used and integrated with use-package.
...
#+begin_src emacs-lisp
  ...
  (defun phd/org-custom-id-get (&optional pom create prefix)
  ...
  (defun phd/org-add-ids-to-headlines-in-file ()
    ...
    (save-excursion
      ...
      (when (re-search-forward "^#\\+OPTIONS:.*auto-id:t" (point-max) t)
        (org-map-entries (lambda () (phd/org-custom-id-get (point) 'create))))))
              ...
              (add-hook 'before-save-hook
                        (lambda ()
                          (when (and (eq major-mode 'org-mode)
                            ...
                            (phd/org-add-ids-to-headlines-in-file))))))
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c r")   'org-ref-citation-hydra/body)
  (global-set-key (kbd "C-c b")   'org-ref-bibtex-hydra/body)
#+end_src

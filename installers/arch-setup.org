...
**** rofi
   ...
   For wayland support install the ~lbonn~ fork.

   #+begin_src  shell
     yay -S rofi-lbonn-wayland
   #+end_src

   Otherwise, use the standard ~rofi~ package
   #+begin_src  shell
     sudo pacman -S rofi
   #+end_src
...
** Hyprland
   ...
   #+begin_src shell
     sudo pacman -S hyprland xdg-desktop-portal-hyprland dunst pipewire wireplumber polkit-kde-agent qt5-wayland qt6-wayland
   #+end_src

   If you use an nvidia card, you might want to try the package including the nvidia patches instead
   #+begin_src shell
     sudo pacman -S qt5ct libva
     yay -S hyprland-nvidia-github
   ...
   #+begin_src shell
     yay -S swayosd-git hyprpaper nwg-look imv grim slurp satty-bin hyprpicker wtype wl-clipboard cliphist swayidle swaylock-effects
   #+end_src
...
** Optional: display manager
   ...
   probably would be ~lightdm~, ~sddm~, or ~ly~. So,
   ...
   Likewise for ~sddm~ in ~/etc/sddm.conf.d/*.conf~

   Additionally, on personal machines it's also nice to have the
   display manager pull some information from the user accounts. For
   this, install
...
*** Password utilities
    ...
    #+begin_src shell
      sudo pacman -S pass pass-otp zbar browserpass browserpass-chromium browserpass-firefox
      yay -S enpass-bin
    #+end_src
...
*** Software for work
    ...
    #+begin_src shell
      python -m venv openconnect
      openconnect/bin/python -m pip install openconnect-sso pyside6
    #+end_src
